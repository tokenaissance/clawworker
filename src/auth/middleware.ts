...
export function createAccessMiddleware(options: AccessMiddlewareOptions) {
  ...
  return async (c: Context<AppEnv>, next: Next) => {
    ...
    // If CF Access is not configured, skip this middleware entirely
    // (caller should have already checked if CF Access is desired)
    if (!teamDomain || !expectedAud) {
      console.log('[AUTH] CF Access credentials not configured, skipping auth');
      return next();
    }
